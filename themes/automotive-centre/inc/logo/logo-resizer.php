 ...
 */
function automotive_centre_logo_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control( 'logo_size', array(
		'label'       => esc_html__( 'Logo Size','automotive-centre' ),
  ...
  $wp_customize->add_setting('automotive_centre_site_title_font_size',array(
		'default'=> '',
		'sanitize_callback'	=> 'sanitize_text_field'
	));
	$wp_customize->add_control('automotive_centre_site_title_font_size',array(
		'label'	=> __('Site Title Font Size','automotive-centre'),
      ...
      'placeholder' => __( '10px', 'automotive-centre' ),
		...
		'section'=> 'title_tagline',
		'type'=> 'text',
		'active_callback' => 'automotive_centre_logo_title_hide_show'
	));
  ...
  $wp_customize->add_setting('automotive_centre_site_tagline_font_size',array(
		'default'=> '',
		'sanitize_callback'	=> 'sanitize_text_field'
	));
	$wp_customize->add_control('automotive_centre_site_tagline_font_size',array(
		'label'	=> __('Site Tagline Font Size','automotive-centre'),
		'description'	=> __('Enter a value in pixels. Example:20px','automotive-centre'),
		'input_attrs' => array(
      'placeholder' => __( '10px', 'automotive-centre' ),
    ),
    'priority'    => 12,
		'section'=> 'title_tagline',
		'type'=> 'text',
		'active_callback' => 'automotive_centre_tagline_hide_show'
	));
}
 ...
 */
function automotive_centre_customize_logo_resize( $html ) {
	...
	if ( is_numeric( $size ) && is_numeric( $custom_logo_id ) ) {
		...
		if ( $logo['width'] >= $logo['height'] ) {
			$output = automotive_centre_min_max( $logo['height'], $logo['width'], $max['height'], $max['width'], $size, $min );
			$img = array(
				'height'	=> $output['short'],
				'width'		=> $output['long']
			);
		// portrait
			...
			$output = automotive_centre_min_max( $logo['width'], $logo['height'], $max['width'], $max['height'], $size, $min );
			$img = array(
				'height'	=> $output['long'],
				'width'		=> $output['short']
			);
...
add_filter( 'get_custom_logo', 'automotive_centre_customize_logo_resize' );

/* Helper function to determine the max size of the logo */
function automotive_centre_min_max( $short, $long, $short_max, $long_max, $percent, $min ){
	$ratio = ( $long / $short );
 ...
 */
function automotive_centre_customize_preview_js() {
	wp_enqueue_script( 'automotive-centre-customizer', esc_url(get_template_directory_uri()) . '/inc/logo/js/customize-preview.js', array( 'jquery', 'customize-preview' ), '201709081119', true );
}
add_action( 'customize_preview_init', 'automotive_centre_customize_preview_js' );

/**
 * JS handlers for Customizer Controls
 */
function automotive_centre_customize_controls_js() {
	wp_enqueue_script( 'automotive-centre-customizer-controls', esc_url(get_template_directory_uri()) . '/inc/logo/js/customize-controls.js', array( 'jquery', 'customize-preview' ), '201709071000', true );
}
add_action( 'customize_controls_enqueue_scripts', 'automotive_centre_customize_controls_js' );

/**
 * Testing function to remove logo_size theme mod
 */
function automotive_centre_remove_theme_mod() {
	if ( isset( $_GET['remove_logo_size'] ) && 'true' == $_GET['remove_logo_size'] ){
		set_theme_mod( 'logo_size', '' );
	}
}
add_action( 'wp_loaded', 'automotive_centre_remove_theme_mod' );