...
<article id="post-<?php the_ID(); ?>" <?php post_class('inner-service'); ?>>
    <div class="single-post">
        <h1><?php the_title(); ?></h1>
        <?php if( get_theme_mod( 'automotive_centre_toggle_postdate',true) != '' || get_theme_mod( 'automotive_centre_toggle_author',true) != '' || get_theme_mod( 'automotive_centre_toggle_comments',true) != '' || get_theme_mod( 'automotive_centre_toggle_time',true) != '') { ?>
            <div class="post-info">
                ...
                <?php if(get_theme_mod('automotive_centre_toggle_author',true)==1){ ?>
                  <span><?php echo esc_html(get_theme_mod('automotive_centre_single_post_meta_field_separator', '|'));?></span> <i class="far fa-user"></i><span class="entry-author"><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' )) ); ?>"><?php the_author(); ?><span class="screen-reader-text"><?php the_author(); ?></span></a></span>
                <?php } ?>

                <?php if(get_theme_mod('automotive_centre_toggle_comments',true)==1){ ?>
                  <span><?php echo esc_html(get_theme_mod('automotive_centre_single_post_meta_field_separator', '|'));?></span> <i class="fa fa-comments" aria-hidden="true"></i><span class="entry-comments"><?php comments_number( __('0 Comment', 'automotive-centre'), __('0 Comments', 'automotive-centre'), __('% Comments', 'automotive-centre') ); ?> </span>
                <?php } ?>

                <?php if(get_theme_mod('automotive_centre_toggle_time',true)==1){ ?>
                  <span><?php echo esc_html(get_theme_mod('automotive_centre_single_post_meta_field_separator', '|'));?></span> <i class="fas fa-clock"></i><span class="entry-time"><?php echo esc_html( get_the_time() ); ?></span>
                <?php } ?>
            </div>
        <?php } ?>
